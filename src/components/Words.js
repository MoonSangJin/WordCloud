...
class Words extends React.Component {
    ...
    _get() { //데이터 베이스 url의 words.json에 접속해서 데이터 가져오는 함수
        fetch(`${databaseURL}/words.json`)
            .then(res => {
                if (res.status != 200) { //상태코드 200번이 아니라면 => 파이어베이스 서버의 오류 발생
    ...
    _post(word) { //firebase는 rest api 형식 지원 따라서 database url에 post 방식으로 접근하여 기입하면 데이터 자체가 json 형태로 기입됨
        return fetch(`${databaseURL}/words.json`, {
            ...
            .then(data => { //새로 추가된 정보를 화면에 보여주기 위한 부분
            ...
            <div>
                {
                    Object
                        ...
                        .map(id => {
                            const word = this
                                .state
                                .words[id]; //word는 json에서 가져온 세개가 들어있는 한 묶음
                            return (
                                <div key={id}>
                                    <Card>
                                        <CardContent>
