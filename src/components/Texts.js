...
class Texts extends React.Component {
    ...
    */
    ...
    handleSubmit = () => {
        const text = { /*우리가 보낼 내용은 textName과 textContent로 구성 */
    ...
    handleFileChange = (e) => { /*사용자가 파일 업로드시 나오는 함수 */
    ...
    render() { //위에서 만든 함수를 사용해서 텍스트파일 등록,삭제하는 디자인을 만들어보자
        const {classes} = this.props;
        return (
            <div> {/* jsx의 기본: 여러개가 들어가면 div하나로 감싸야됨 */}
                {
                    Object
                        .keys(this.state.texts)
                        .map(id => {
                            const text = this //text state에 포함되어있는 모든 원소를 map함수로 id 기준으로 방문하면서 카드 형태로 출력
                                .state
                                .texts[id];
                            return (
                                <Card key={id}>
                                    <CardContent>
                                        <Typography color="textSecondary" gutterBottom="gutterBottom">
                                            내용: {
                                                text
                                                    .textContent
                                                    .substring(0, 24) + '...' //js에서 파일내용자르는 함수 => substring 24자까지 보여줌
                                            }
                                        </Typography>
                                        <Grid container="container"> {/* grid에 container 속성 넣어주어야 보기,삭제 속성이 한줄로 출력됨 */}
                                            <Grid item="item" xs={6}> {/* 6 3 3 즉 2대1대1로 나눠서 보여줄 것 */}
                                            ...
                                            <Grid item="item" xs={3}>
                                                <Typography variant="h5" component="h2">
                                                    <Link component={RouterLink} to={"detail/" + id}> {/* 버튼 눌렀을때 detail경로 즉 id 값으로 이동할수있게 */}
